...
public class DataConverter : MonoBehaviour
{
    ...
    private void ConvertStationData()
    {
        ...
        for (int j = 0; j < lineManager.lineCollections[4].lineData.hasStation.Length; j++) // 5호선 매칭
        ...
        for (int j = 0; j < lineManager.lineCollections[5].lineData.hasStation.Length; j++) // 6호선 매칭
            tmp[j] = PlayerPrefs.GetInt("bool_stations6[" + j + "]").Equals(1) ? true : false;
        lineManager.lineCollections[5].lineData.hasStation[0] = tmp[38];
        for (int j = 1; j < lineManager.lineCollections[5].lineData.hasStation.Length; j++) // 6호선 매칭
